...
#define DEVICE_NAME                     "Apollo"                               /**< Name of device. Will be included in the advertising data. */
...
enum freqs_t freq;
enum ears_t ear;
enum states_t state = notListening;
...
static void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
  switch(state) {
		case notListening:
			if (*p_data == STARTLISTENING) {
				state = listeningForFreq;
			}
				break;
		case listeningForFreq:
			freq = (enum freqs_t)(*p_data);	//TODO: put some protection here so we stay in bounds
		...
		case listeningForEar:
			ear = (enum ears_t)(*p_data);
			state = notListening;
			printf("%d freq, %d vol, %d ear\n", freq, masterVolume, ear);
			break;
	}
...
//		uint8_t moredata[] = "MoreData!";
//		manuf_data_response.data.p_data = moredata;
//		manuf_data_response.data.size = sizeof(moredata);
